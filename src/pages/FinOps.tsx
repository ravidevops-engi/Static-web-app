...
import { DollarSign, ChevronDown, Bookmark } from "lucide-react";
import { finOpsSections } from "@/data/finOpsData";
...
export default function FinOps() {
  ...
  const { viewedCount, bookmarkedCount, markAsViewed, toggleBookmark, isBookmarked } =
    useQuestionProgress("finops");

  const totalQuestions = finOpsSections.reduce(
  ...
  return (
    <div className="container max-w-4xl px-4 py-12">
      ...
      <div className="mb-12 flex items-center gap-4">
        <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-500 shadow-glow">
          <DollarSign className="h-8 w-8 text-white" />
        </div>
        <div>
          <h1 className="text-4xl font-bold">Monitoring & FinOps</h1>
          <p className="text-lg text-muted-foreground">
            Cost Optimization, Cloud Governance & Financial Operations
          </p>
        </div>
      </div>
      ...
      <div className="prose prose-slate dark:prose-invert max-w-none">
        ...
        <p className="text-lg mb-8">
          Master FinOps practices, cost optimization strategies, and cloud
          financial management.
        </p>

        <Accordion type="multiple" className="space-y-4">
          {finOpsSections.map((section, index) => (
            <AccordionItem
              key={index}
              value={`section-${index}`}
