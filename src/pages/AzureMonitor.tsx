...
import { Activity, ChevronDown, Bookmark } from "lucide-react";
import { azureMonitorSections } from "@/data/azureMonitorData";
...
export default function AzureMonitor() {
  ...
  const { 
    ...
    toggleBookmark, 
    isBookmarked 
  } = useQuestionProgress("azure-monitor");

  const totalQuestions = azureMonitorSections.reduce((acc, section) => acc + section.questions.length, 0);
  ...
  return (
    <div className="container max-w-4xl px-4 py-12">
      <div className="mb-12 flex items-center gap-4">
        ...
        <div>
          <h1 className="text-4xl font-bold">Azure Monitor & Observability</h1>
          <p className="text-lg text-muted-foreground">
            Monitoring, Logging, Alerts & Observability Fundamentals
          </p>
        </div>
      </div>
      ...
      <div className="prose prose-slate dark:prose-invert max-w-none">
        ...
        <p className="text-lg mb-8">
          Master Azure Monitor, KQL queries, Prometheus, Grafana, and complete observability practices.
        </p>

        <Accordion type="multiple" className="space-y-4">
          {azureMonitorSections.map((section, index) => (
            <AccordionItem
              key={index}
              value={`section-${index}`}
              ...
              <AccordionContent>
                <div className="space-y-4 mt-4">
                  {section.questions.map((item, qIndex) => {
                    ...
                    return (
                      <div key={qIndex} className="border-l-2 border-purple-500/30 pl-4 py-2">
