...
import { Cloud, ChevronDown, Bookmark } from "lucide-react";
import { azureSections } from "@/data/azureData";
...
export default function Azure() {
  ...
  const {
    ...
    toggleBookmark,
    isBookmarked
  } = useQuestionProgress("azure");

  const totalQuestions = azureSections.reduce((acc, section) => acc + section.questions.length, 0);
  ...
  return (
    <div className="container max-w-4xl px-4 py-12">
      <div className="mb-12 flex items-center gap-4">
        <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-blue-500 to-cyan-500 shadow-glow">
          <Cloud className="h-8 w-8 text-white" />
        </div>
        <div>
          <h1 className="text-4xl font-bold">Azure Core</h1>
          <p className="text-lg text-muted-foreground">
            Microsoft Azure Cloud Platform Interview Questions
          </p>
        </div>
      </div>
      ...
      <div className="prose prose-slate dark:prose-invert max-w-none">
        ...
        <p className="text-lg mb-8">
          Comprehensive Azure interview questions covering fundamentals, compute, networking,
          storage, security, and identity management.
        </p>

        <Accordion type="multiple" className="space-y-4">
          {azureSections.map((section, index) => (
            <AccordionItem
              key={index}
              value={`section-${index}`}
                ...
                <div className="space-y-4 mt-4">
                  {section.questions.map((item, qIndex) => {
                    ...
                    const hasPlainAnswer =
                      typeof item === "object" && item && "answer" in item && !!(item as any).answer;
                    const hasHtmlAnswer =
                      typeof item === "object" && item && "answerHtml" in item && !!(item as any).answerHtml;

                    const answerText = hasPlainAnswer ? (item as any).answer : null;
                    const hasAnyAnswer = hasPlainAnswer || hasHtmlAnswer;

                    return (
                      <div key={qIndex} className="border-l-2 border-secondary/30 pl-4 py-2">
